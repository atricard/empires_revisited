...
summon glow_item_frame ~ ~ ~ {CustomName:'{"text":"Building Display (BUG)"}',CustomNameVisible:0b,Facing:1b,ItemRotation:0b,Invulnerable:1b,Invisible:1b,Fixed:1b,Silent:1b,Tags:["empbl","empbl_building","empbl_inactive","new","blocks_only","garden"],Item:{id:"minecraft:structure_void",Count:1b,tag:{CustomModelData:440001}}}
setblock ~ ~ ~ oak_sign{Text1:'[{"text":"Garden ","color":"green"},{"text":"(","color":"gray"},{"selector":"@a[scores={emp_use_allay_egg=1..},distance=..10]}]","color":"gray"},{"text":")","color":"gray"}]'} replace
