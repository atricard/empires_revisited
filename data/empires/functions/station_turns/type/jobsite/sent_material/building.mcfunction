...
#Message
execute as @a[predicate=empires:message_id] run tellraw @s [{"text":"<⚑>","color":"#00BFFF"},{"text":" Turn ","color":"white"},{"score":{"name":"@s","objective":"emp_turn"},"color":"white"},{"text":": ","color":"white"},{"selector":"@e[type=marker,tag=emp_sent_worker,sort=nearest,limit=1]","color":"#f4a460"},{"text":" sent materials to a storage building.","color":"white"}]
#Send
